...
Hacker_News_Scraper is deployed on the following stack.
...
documented on that page, you need to create a new file in ``hacker_news_scraper/celery.py`` and then
modify ``hacker_news_scraper/__init__.py`` to import that file. You'll also need to customize ``{{
...
``requirements/base.txt`` and installed it)::
    ...
    celery -A hacker_news_scraper worker

Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::

    # hacker_news_scraper/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://hacker_news_scraper_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/hacker_news_scraper_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``hacker_news_scraper_<environment>`` and the
vhost will be named ``hacker_news_scraper_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out hacker_news_scraper.key 2048
  # Make copy of the key with the passphrase
  cp hacker_news_scraper.key hacker_news_scraper.key.secure
  # Remove any passphrase
  openssl rsa -in hacker_news_scraper.secure -out hacker_news_scraper.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key hacker_news_scraper.key -out hacker_news_scraper.csr
